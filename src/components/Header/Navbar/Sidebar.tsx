...
function Sidebar({toggleHandler} : SidebarProps) {
  ...
  return (
    <motion.div
      className='h-[100dvh] w-screen absolute top-0 right-0'
      initial={{ x: 300 }} 
      animate={{x:0}} 
    ...
    >
      ...
      {/* mainSidebar */}
      <div className='w-72 h-full bg-white absolute top-0 right-0 flex flex-col justify-between rounded-l-lg border-2 border-blue-800 '>
        
        <div className='h-16 flex gap-1 border-b-2 border-gray-200 px-4 py-3.5' onClick={() => router.push("/")}>
        ...
        <ul className={`${isAuthenticated? 'h-full gap-5' : ' h-full gap-3'} flex flex-col font-medium  px-2 bg-blue-600 pt-3`}>
        ...
        {
          !isAuthenticated &&
            <div className=' h-20 flex flex-row justify-evenly p-4 border-t-2 border-white '>
